...
const HeavyMachinesForm = ({ sector, data, setData }) => {
  ...
  const inputClass =
    "px-2 py-1 border rounded-md text-sm dark:bg-gray-900 dark:border-gray-700 dark:text-white transition-colors duration-500 ease-in-out";
    ...
    <div>
      <div className="flex justify-start mb-1">
        <button
          type="button"
          onClick={() => {
            // Vérifie si toutes les machines sont déjà RAS
            const allRAS = machines.every((m) => {
              ...
              return (
                [
                  ...
                  "controle",
                  "pression",
                ].includes(m.type) && current?.ras === true
              );
            });
      ...
      <table className="w-full text-sm text-left border-separate border-spacing-y-2 text-gray-800 dark:text-gray-200">
        ...
        <tbody>
          {machines.map((m) => (
            <tr
              key={m.tag}
              className="group hover:bg-gray-200 dark:hover:bg-gray-700 bg-gray-100 dark:bg-gray-800 transition-colors duration-500 ease-in-out relative"
            >
              <td className="w-[10%] py-2 pl-3 font-semibold rounded-l-md relative group border-r border-gray-300 dark:border-gray-600">
                ...
                <button
                  type="button"
                  onClick={() =>
                    document.getElementById(`image-${m.tag}`)?.click()
                  }
              ...
              <td className="py-2 pl-4 pr-2 rounded-r-md">
                {m.type === "p211" && (
                  <div className="flex flex-wrap gap-2 items-center">
                    <label className="flex items-center gap-2">
                      ...
                      <input
                        type="text"
                        placeholder="bar"
                        value={data[m.tag]?.pression || ""}
                        onChange={(e) =>
                          handleChange(m.tag, "pression", e.target.value)
                        }
                    ...
                    <label className="flex items-center gap-2">
                      ...
                      <input
                        type="text"
                        placeholder="°C"
                        value={data[m.tag]?.temperature || ""}
                        onChange={(e) =>
                          handleChange(m.tag, "temperature", e.target.value)
                        }
                        className={inputSmall}
                      />
                    </label>
                    ...
                    <label className="flex items-center gap-2">
                      ...
                      <input
                        type="text"
                        placeholder="hh:mm"
                        value={data[m.tag]?.heure || ""}
                        onChange={(e) =>
                          handleChange(m.tag, "heure", e.target.value)
                        }
                        className={inputSmall}
                      />
                    </label>
                ...
                {m.type === "vidange" && (
                  <label className="flex items-center gap-2">
                    <span className={labelText}>
                      Vidange entretoise faite le :
                    </span>
                    <DatePicker
                      selected={
                        data[m.tag]?.vidange
                          ? new Date(data[m.tag].vidange)
                          : null
                      }
                      onChange={(date) =>
                        handleChange(
                ...
                {m.type === "controle" && (
                  <div className="flex flex-wrap gap-4 items-center">
                    ...
                    <label className="inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={data[m.tag]?.circulation || false}
                        onChange={(e) =>
                          handleChange(m.tag, "circulation", e.target.checked)
                        }
                        className="hidden"
                      />
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium transition-colors duration-200 ${
                          data[m.tag]?.circulation
                            ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border border-green-300 dark:border-green-600"
                            : "bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400 border border-gray-300 dark:border-gray-600"
                        }`}
                      >
                    ...
                    <label className="inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={data[m.tag]?.niveau || false}
                        onChange={(e) =>
                          handleChange(m.tag, "niveau", e.target.checked)
                        }
                        className="hidden"
                      />
                      <span
                ...
                {/* ✅ Observation complémentaire (même pour machines avec mesures) */}
                {["p211", "vidange", "controle", "pression"].includes(
                  m.type
                ) && (
                  <div className="mt-2 flex items-center gap-2">
                    <label className="inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={data[m.tag]?.ras || false}
                        onChange={(e) =>
                          handleChange(m.tag, "ras", e.target.checked)
                        }
                        className="hidden"
                      />
                      <span
                        className={`px-2 py-1 rounded-full text-xs text-center font-medium transition-colors duration-200 ${
                          data[m.tag]?.ras
                            ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border border-green-300 dark:border-green-600"
                            : "bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400 border border-gray-300 dark:border-gray-600"
                        }`}
                    ...
                    <textarea
                      disabled={data[m.tag]?.ras}
                      placeholder="Observation / Anomalie complémentaire"
                      value={data[m.tag]?.comment || ""}
                      onChange={(e) =>
                        handleChange(m.tag, "comment", e.target.value)
                      }
                      rows={1}
                      className={`${inputFull} resize-none ${
                        data[m.tag]?.ras ? "opacity-50 cursor-not-allowed" : ""
                      }`}
                      onInput={(e) => {
